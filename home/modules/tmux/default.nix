...
let
    ...
    (
      ...
      (
        let
          mapFlags =
            flags:
            "-" + (builtins.concatStringsSep "" (builtins.map (flagName: optionFlags.${flagName}) flags));
        in
        (concatStringsSep "\n" (
          map (option: "set-option ${mapFlags option.flags} ${option.name} ${option.value}") options
        ))
      )
    );
  ...
  mapKeyBinds =
    ...
    (
      command: keys:
      (concatStringsSep "\n" (attrValues (mapAttrs (key: value: "${command} ${key} ${value}") keys)))
    );

  mapKeyUnbinds = with builtins; (keys: (concatStringsSep "\n" (map (key: "unbind ${key}") keys)));
