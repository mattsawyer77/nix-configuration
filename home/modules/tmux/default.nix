{
  ...
  mapOptions = with builtins; (options: (
    let
      mapFlags = flags:
        "-"
    ...
    in (concatStringsSep "\n" (map
      (option: "set-option ${mapFlags option.flags} ${option.name} ${option.value}")
      options))
  ));
  ...
  mapKeyBinds = with builtins; (command: keys: (concatStringsSep "\n" (attrValues
