...
in {
  options.custom.git = {
    defaultEmail = lib.mkOption {
      type = lib.types.str;
      description = "Default email for git commits.";
    };
  ...
  config = {
    ...
    programs.git = {
      ...
      aliases = {
        ...
        lt = "log --tags --simplify-by-decoration --format='%C(green)%H %<(15)%C(yellow)%ci %<(20,trunc)%C(cyan)%aN %C(reset)%<(200,trunc)%d%n   %s'";
        st = "status -s";
      };
      delta = {
        enable = true;
